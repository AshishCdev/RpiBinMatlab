...
void HelloWorld_step(void)
{
  ...
  if (HelloWorld_DW.is_active_c3_HelloWorld == 0U) {
    ...
    /* Transition: '<S2>:10' */
    if (HelloWorld_DW.count1 > 10000000U) {
      /* Transition: '<S2>:14' */
      /* Transition: '<S2>:16' */
  ...
  /* Outport: '<Root>/OutLed' */
  HelloWorld_Y.OutLed = rtb_OutLed;
...
void HelloWorld_initialize(void)
{
  ...
  /* external inputs */
  HelloWorld_U.InSw = 0.0;

  /* external outputs */
  HelloWorld_Y.OutLed = 0.0;
